...
try:
    ...
    # file doesn't exist or is corrupted - write default_time_control to it and set time_control
    # to default
...
def update_display():
    # naively write text_buffer to the lcd
    # TODOMAYBE: could upgrade to only write chars that have changed?
    for row_n in [0, 1]:
        lcd.move_to(0, row_n)
        lcd.putstr(text_buffer[row_n])
        # for col_n in range(16):
        #     lcd.move_to(col_n, row_n)
        #     ch = text_buffer[row_n][col_n]
        #     lcd.putchar(ch)


def blink(row_n: int, col_n: int):
    def revert_char():
        lcd.move_to(
            row_n,
            col_n,
...
def debounced(handler, interval_ms):
    ...
    def wrapper(*args, **kwargs):
        ...
        if time.ticks_diff(current_time, previous_call_time[0]) > interval_ms:
            previous_call_time[0] = current_time
...
try:
    ...
    def set_setting_up_time_control_to_false():
        # ... lambda can't have expression?? xddddddddddd
        global setting_up_time_control
        setting_up_time_control = False

    update_text_buffer_initialisation_stage()
    update_display()
    # set up stage for changing the type
    blink_timer = get_blink_timer([(6, 0), (7, 0), (8, 0), (9, 0), (10, 0)])
    ...
    button2.irq(
        handler=lambda _pin: set_setting_up_time_control_to_false(),
        trigger=Pin.IRQ_FALLING,
    )
